...
void process_throttle(uint16_t fwd, uint16_t rvd)
{
	...
	{
		metrics.direction = 1;
		metrics.throttle = fwd;
		metrics.pwm = throttle_fwd;
	}
	else
	{
		metrics.direction = -1;
		metrics.throttle = rvd;
		metrics.pwm = throttle_rvd;
	}
	...
	ESP_LOGI(TAG, "Throttle=%i (%i), %i (%i)", throttle_fwd, fwd, throttle_rvd, rvd);
	ESP_ERROR_CHECK(ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_1, throttle_fwd));
	ESP_ERROR_CHECK(ledc_update_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_1));
	ESP_ERROR_CHECK(ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_0, throttle_rvd));
...
void process_state(ps5_t *data)
{
	...
	process_throttle(data->analog.button.r2, data->analog.button.l2);
	...
	bool reverse = (metrics.direction == -1) ? 1 : 0;
	bool fwd = (metrics.direction == 1) ? 1 : 0;
